...
export async function createPlayer(name: string, skill: number): Promise<Player> {
const newPlayer = {
...
export async function updatePlayer(id: string, name: string, skill: number): Promise<Player | null> {
...
export async function updateTeam(id: string, name: string): Promise<Team | null|string> {
const index = teams.findIndex((team) => team.id === id)
if (index === -1) return null

const teamNames = await getTeams()
const hasDuplicate = teamNames.some((team, i) => team?.name === name && i !== index); 
...
// Team generation
export async function generateTeams(title: string): Promise<GeneratedSession | null | string> {
...
for (const team of teamAssignments) {
    ...
    team.averageSkill = Number.parseFloat((totalSkill / team.players.length).toFixed(2))
    }
}
...
export async function getSessionByPublicId(publicId: string): Promise<GeneratedSession | undefined> {
